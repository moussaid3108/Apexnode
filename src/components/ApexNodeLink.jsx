...
header{position:relative;z-index:1;text-align:center;padding:56px 0 26px}
.eyebrow{font-family:'Space Mono',monospace;font-size:12px;letter-spacing:.55em;color:var(--lav);text-indent:.55em}
h1{margin-top:10px;font-weight:700;font-size:clamp(40px,11vw,84px);letter-spacing:.08em;line-height:1;background:linear-gradient(100deg,var(--lav) 15%,var(--blue) 85%);-webkit-background-clip:text;background-clip:text;color:transparent;filter:drop-shadow(0 2px 20px rgba(0,0,0,0.85))}
.hub{margin-top:8px;font-weight:500;font-size:clamp(20px,5vw,34px);letter-spacing:.6em;text-indent:.6em;color:var(--violet)}
.tagline{margin:22px auto 0;max-width:640px;padding:0 24px;font-family:'Space Mono',monospace;font-size:clamp(10px,2.8vw,13px);letter-spacing:.3em;line-height:2;color:var(--ink-dim);text-transform:uppercase;text-shadow:0 1px 10px rgba(0,0,0,0.9)}
...
export default function ApexNodeLink() {
  ...
  useEffect(() => {
    ...
    const hexMat = new THREE.MeshStandardMaterial({ color:0x14102A, metalness:0.7, roughness:0.25, emissive:0x2A1E55, emissiveIntensity:0.3, transparent:true, opacity:0.55 });
    const hex = new THREE.Mesh(hexGeo, hexMat);
    hex.rotation.x = Math.PI / 2;
    core.add(hex);
    const hexEdges = new THREE.LineSegments(new THREE.EdgesGeometry(hexGeo), new THREE.LineBasicMaterial({ color:0xB7A8F5, transparent:true, opacity:0.55 }));
    hexEdges.rotation.copy(hex.rotation);
    core.add(hexEdges);

    const ringA = new THREE.Mesh(new THREE.TorusGeometry(2.05, 0.045, 16, 100), new THREE.MeshStandardMaterial({ color:0x8B5CF6, emissive:0x8B5CF6, emissiveIntensity:0.35, metalness:0.6, roughness:0.3 }));
    core.add(ringA);
    const ringB = new THREE.Mesh(new THREE.TorusGeometry(2.35, 0.02, 16, 100), new THREE.MeshStandardMaterial({ color:0x4F9CF9, emissive:0x4F9CF9, emissiveIntensity:0.4, metalness:0.6, roughness:0.3 }));
    ringB.rotation.x = Math.PI / 3;
    core.add(ringB);
    const ringC = new THREE.Mesh(new THREE.TorusGeometry(2.7, 0.014, 12, 100), new THREE.MeshStandardMaterial({ color:0xC4B5FD, emissive:0xC4B5FD, emissiveIntensity:0.3, metalness:0.5, roughness:0.4 }));
    ...
    const sats = [];
    for (let i = 0; i < 5; i++) {
      const m = new THREE.Mesh(satGeo, new THREE.MeshStandardMaterial({ color:satColors[i], emissive:satColors[i], emissiveIntensity:0.45, metalness:0.6, roughness:0.3 }));
      satGroup.add(m);
    ...
    function updateScan(t) {
      ...
      hexMat.emissiveIntensity = 0.3 + near*0.9;
      ringA.material.emissiveIntensity = 0.35 + near*0.5;
      ringB.material.emissiveIntensity = 0.4 + near*0.4;
    }
    ...
    function animate() {
      ...
      if (headerEl) {
        ...
        if (rect.bottom > 0) {
          const cy = 1 - 2*(rect.top + rect.height*0.5)/window.innerHeight;
          _aN.set(0, cy, -1).unproject(camera);
          _aF.set(0, cy,  1).unproject(camera);
          _aD.copy(_aF).sub(_aN).normalize();
          const tv = Math.abs(_aD.z) > 0.001 ? -_aN.z/_aD.z : 0;
          coreBaseY = _aN.y + tv * _aD.y;
        }
      }
      ...
      if (!reduced) {
        ...
        core.position.y = coreBaseY + Math.sin(t*0.8)*0.1;
